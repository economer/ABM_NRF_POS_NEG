...
def run_sim(num_steps, flat_benefit_amount=0):
    ...
    lim = 24263 / 26
    lic = 20386 / 26
    base_benefit_values = [mbm]  # Include all base benefit levels
    ...
    for base_benefit in base_benefit_values:
        for nit_r in nit_r_values:
            model = IncomeModel(nit_r, base_benefit, data, consumption_types, flat_benefit_amount=0)  # Regular benefits
            for _ in range(num_steps):
                model.step()
            ...
            df['scenario'] = f'base_benefit_{base_benefit}_nit_r_{nit_r}'  # Mark these results with their scenario
